...
define('FILENAME_ADVANCED_SEARCH_RESULT', 'catalog/advanced-search-result');
...
/* }} */
if (isset($_SERVER['REQUEST_URI']) || isset($HTTP_SERVER_VARS['REQUEST_URI'])) $script_pos = $HTTP_SERVER_VARS['REQUEST_URI'] = $_SERVER['REQUEST_URI'] = (isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : $HTTP_SERVER_VARS['REQUEST_URI']);
if (!isset($script_pos)) $script_pos = (isset($_SERVER['PHP_SELF']) ? preg_replace ("/.php/i", "", $_SERVER['PHP_SELF']) : preg_replace ("/.php/i", "", $HTTP_SERVER_VARS['PHP_SELF']));
//if (basename($script_pos) == 'order-process') {
if (strpos($_SERVER['REQUEST_URI'], 'order-process') !== false || ( (isset($_POST['M_paid']) && $_POST['M_paid'] == 'partlypaid') || (isset($HTTP_POST_VARS['M_paid']) && $HTTP_POST_VARS['M_paid'] == 'partlypaid') )){
define('FILENAME_CHECKOUT_PROCESS', 'account/order-process');
define('FILENAME_CHECKOUT_PAYMENT', 'account/order-pay');
} else {
...
define('FILENAME_PRODUCTS_NEW', 'catalog/products_new');
...
define('FILENAME_SPECIALS', 'catalog/specials');
...
// Lango Added for ALL_PODS Mod: BOF
define('FILENAME_ORDERS_PRINTABLE', 'email-template/invoice');
// Lango Added for ALL_PRODS Mod: EOF
// Lango Added for Featured Products: BOF
define('FILENAME_FEATURED', CONTENT_FEATURED . '.php');
define('FILENAME_FEATURED_PRODUCTS', 'catalog/featured_products');
