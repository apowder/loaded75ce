...
class recurly extends ModulePayment {
    ...
    function update_status() {
        ...
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_RECURLY_SERVER_ZONE > 0)) {
            $check_flag = false;
            $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_RECURLY_SERVER_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function before_process() {
        ...
        try {
            ...
            $account->account_code = $customer_id;
            ...
            case 'renewed_subscription_notification'://Renewed Subscription
                ...
                switch ($subscription->__get('state')) {
                    case 'active':
                        ...
                        break;
                    case 'canceled':
                        $current_status = 100003;
                        break;
                    case 'future':
                        $current_status = 100004;
                        break;
                    case '100005':
                        $current_status = 100005;
                        break;
                    default:
                        break;
                }
