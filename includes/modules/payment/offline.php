...
class offline extends ModulePayment {
    ...
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_OFFLINE_ZONE > 0) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_OFFLINE_ZONE . "' and zone_country_id = '" . $order->delivery['country']['id'] . "' order by zone_id");
  ...
  `status` int(1) NOT NULL DEFAULT '1'
    ...
    function confirmation() {
        global $payment, $languages_id;

        $description = '';
        $code = explode('_', $payment);
        if (isset($code[1])) {
            $platform_id = PLATFORM_ID;
