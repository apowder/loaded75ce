...
// class methods
    function quote($method = '') {
      global $order, $cart, $total_count, $currencies;
      $dest_zone = 0;
      $error = false;

      for ($i=1; $i<=$this->num_zonesperitem; $i++) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . (int)constant('MODULE_SHIPPING_ZONESPERITEM_GEO_ZONE_' . $i) . "' and zone_country_id = '" . (int)$order->delivery['country']['id'] . "' order by zone_id");
