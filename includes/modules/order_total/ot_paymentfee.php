...
class ot_paymentfee extends ModuleTotal {
    ...
    public function process($replacing_value = -1, $visible = false) {
        ...
        $pass = false;
        if (MODULE_ORDER_TOTAL_PAYMENTFEE_ALLOW_FEE == 'true') {
            ...
            if ($pass || $replacing_value != -1) {
                ...
                $module = $order->info['payment_class'];
                ...
                if (tep_db_num_rows($check_query) > 0) {
                    ...
                    $of_amount += ($check['percent_prefix'] * $order->info['subtotal_exc_tax'] * ($check['percent_value'] / 100));
                }

                $of_amount = round($of_amount, 2);

                $tof_amount = $of_amount + \common\helpers\Tax::calculate_tax($of_amount, $tax);
...
//                $tof_amount = round($tof_amount, 2);
    ...
    public function configure_keys() {
        return array(
            ...
            'MODULE_ORDER_TOTAL_PAYMENTFEE_ALLOW_FEE' =>
            array(
                'title' => 'Allow Payment Fee',
                'value' => 'false',
                'description' => 'Do you want to allow payment fees?',
                'sort_order' => '3',
                'set_function' => 'tep_cfg_select_option(array(\'true\', \'false\'), ',
            ),
  ...
  PRIMARY KEY (`payment_fee_id`)
    ...
    public function extra_params() {
        ...
        for ($i = 0, $n = sizeof($directory_array); $i < $n; $i++) {
            ...
            if (file_exists(DIR_FS_CATALOG_LANGUAGES . 'modules/payment/' . $file)) {
                require_once(DIR_FS_CATALOG_LANGUAGES . 'modules/payment/' . $file);
            }
            require_once(DIR_FS_CATALOG_MODULES . 'payment/' . $file);
...
var id = $(\'select[name="ship_method_code"]\').val();
    ...
    private function directoryList() {
        ...
        if ($dir = @dir(DIR_FS_CATALOG_MODULES . 'payment/')) {
            while ($file = $dir->read()) {
                if (!is_dir(DIR_FS_CATALOG_MODULES . 'payment/' . $file)) {
