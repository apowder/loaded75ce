            ...
            if ($payment == $table[$i]) {
                ...
                if ($od_min_fee < $od_fee) {
                    ...
                    if (DISPLAY_PRICE_WITH_TAX == "true") {
                        ...
                        $order->info['tax_groups']["$tax_description"] += \common\helpers\Tax::calculate_tax($od_am / (1 + ($tod_rate / 100)), $tod_rate);
                    } else {
                        $tod_amount = \common\helpers\Tax::calculate_tax($od_am, $tod_rate);
                        $order->info['tax_groups']["$tax_description"] += \common\helpers\Tax::calculate_tax($od_am, $tod_rate);
