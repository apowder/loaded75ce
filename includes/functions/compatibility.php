...
// Ie, products attributes
  function do_magic_quotes_gpc(&$ar) {
    ...
    while (list($key, $value) = each($ar)) {
      if (is_array($value)) {
        $ar[$key] = do_magic_quotes_gpc($value);
      } else {
  ...
  function do_magic_unquotes_gpc(&$ar) {
    if (!is_array($ar)) return false;
    reset($ar);
    while (list($key, $value) = each($ar)) {
      if (is_array($value)) {
        $ar[$key] = do_magic_unquotes_gpc($value);
      } else {
...
// handle magic_quotes_gpc turned off.
  if (get_magic_quotes_gpc()) {
    do_magic_unquotes_gpc($HTTP_GET_VARS);
    do_magic_unquotes_gpc($HTTP_POST_VARS);
    do_magic_unquotes_gpc($HTTP_COOKIE_VARS);
  }
