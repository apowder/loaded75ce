...
// set php_self in the local scope
  ...
  if (isset($_SERVER['REQUEST_URI']) || isset($HTTP_SERVER_VARS['REQUEST_URI'])) $PHP_SELF = $HTTP_SERVER_VARS['REQUEST_URI'] = $_SERVER['REQUEST_URI'] = (isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : $HTTP_SERVER_VARS['REQUEST_URI']);
  ...
  if (!isset($PHP_SELF)) $PHP_SELF = (isset($_SERVER['PHP_SELF']) ? preg_replace ("/.php/i", "", $_SERVER['PHP_SELF']) : preg_replace ("/.php/i", "", $HTTP_SERVER_VARS['PHP_SELF']));
  $pos = strpos($PHP_SELF, '?');
  if ($pos !== false) {
...
// lets start our session
  ...
  if ( (PHP_VERSION >= 4.3) && function_exists('ini_get') && (ini_get('register_globals') == false || ini_get('register_globals') == 'Off') ) {
...
include('includes/add_pc_conf_application_top.php');
