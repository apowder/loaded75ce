...
// The HTML href link wrapper function
  ...
  function tep_catalog_href_link($page = '', $parameters = '', $connection = 'NONSSL') {
    if ( class_exists('Yii',false) && !Yii::$app->get('platform')->config()->isCatalogBaseUrlWithId() ) {
      $link = Yii::$app->get('platform')->config()->getCatalogBaseUrl( $connection == 'SSL' );
    }else
    if ($connection == 'NONSSL') {
    ...
    if ( Yii::$app->get('platform')->config()->isCatalogBaseUrlWithId() ) {
      $parameters .= (empty($parameters)?'':'&').'platform_id='. Yii::$app->get('platform')->config()->getId();
    }
    if ($parameters == '') {
      $link .= $page;
    } else {
...
// Output a form textarea field
  function tep_draw_textarea_field($name, $wrap, $width, $height, $text = '', $parameters = '', $reinsert_value = true) {
    ...
    $field .= '>';
     ...
     if ( ($reinsert_value == true) && ( (isset($HTTP_GET_VARS[$name]) && is_string($HTTP_GET_VARS[$name])) || (isset($HTTP_POST_VARS[$name]) && is_string($HTTP_POST_VARS[$name])) ) ) {
      if (isset($HTTP_GET_VARS[$name]) && is_string($HTTP_GET_VARS[$name])) {
        $field .= \common\helpers\Output::output_string_protected(stripslashes($HTTP_GET_VARS[$name]));
      } elseif (isset($HTTP_POST_VARS[$name]) && is_string($HTTP_POST_VARS[$name])) {
        $field .= \common\helpers\Output::output_string_protected(stripslashes($HTTP_POST_VARS[$name]));
      }
...
// Output a form hidden field
  function tep_draw_hidden_field($name, $value = '', $parameters = '') {
    ...
    if (tep_not_null($value)) {
      ...
      if ( (isset($HTTP_GET_VARS[$name]) && is_string($HTTP_GET_VARS[$name])) ) {
        $field .= ' value="' . \common\helpers\Output::output_string(stripslashes($HTTP_GET_VARS[$name])) . '"';
      } elseif ( (isset($HTTP_POST_VARS[$name]) && is_string($HTTP_POST_VARS[$name])) ) {
        $field .= ' value="' . \common\helpers\Output::output_string(stripslashes($HTTP_POST_VARS[$name])) . '"';
      }
    }
