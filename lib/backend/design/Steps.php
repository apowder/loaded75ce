...
class Steps
{
  ...
  public static function boxData($id){
    ...
    if ($box['widget_name'] == 'BlockBox' || $box['widget_name'] == 'email\BlockBox' || $box['widget_name'] == 'invoice\Container'){
  ...
  public static function styleSaveUndo($step){
    ...
    foreach ($data['styles'] as $item){
      tep_db_query("delete from " . TABLE_THEMES_STYLES_TMP . " where id = '" . (int)$item['id'] . "'");
    }
    foreach ($data['styles_old'] as $item){
      tep_db_perform(TABLE_THEMES_STYLES_TMP, $item);
    }
  }
  ...
  public static function extendRemove($data){
    ...
    $themes_styles = [];
    $query = tep_db_query("select * from " . TABLE_THEMES_STYLES_TMP . " where visibility = '" . (int)$data['id'] . "'");
    while ($item = tep_db_fetch_array($query)){
      $themes_styles[] = $item;
    }
  ...
  public static function extendRemoveRedo($step){
    ...
    tep_db_query("delete from " . TABLE_THEMES_STYLES_TMP . " where visibility = '" . tep_db_input($data['themes_settings']['id']) . "'");
  ...
  public static function cssSave($data){
    ...
    if ($query['event'] == 'cssSave' && $query['admin_id'] == $_SESSION['login_id']){
      ...
      $data_s = [
        'css_old' => $data['css_old'],
        'css' => $data['css'],
      ];
      self::stepSave('cssSave', $data_s, $data['theme_name']);
  ...
  public static function cssSaveUndo($step){
    ...
    tep_db_perform(TABLE_THEMES_SETTINGS, [
      'setting_value' => $data['css_old']
    ], 'update', " theme_name = '" . tep_db_input($data['theme_name']) . "' and setting_group = 'css' and setting_name = 'css'");
  }
  ...
  public static function log($theme_name){
    ...
    $log = array();
    $query = tep_db_query("select steps_id, parent_id, event, date_added, admin_id from " . TABLE_THEMES_STEPS . " where theme_name='" . tep_db_input($theme_name) . "'");
    while ($item = tep_db_fetch_array($query)){
      $log[$item['steps_id']] = [
  ...
  public static function restore($id){
    ...
    while (!$theme_name){
      ...
      if ($theme_name && $data['backup_id']){
        DesignController::backupRestore($data['backup_id'], $theme_name);
        tep_db_perform(TABLE_THEMES_STEPS, array('active' => '0'), 'update', "active = '1' and theme_name='" . tep_db_input($theme_name) . "'");
