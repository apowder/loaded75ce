...
class Directory extends Object
{
    ...
    {
        return $this->parent_id!=0 && $this->cron_enabled && $this->removable && in_array($this->directory_type, [self::TYPE_DATASOURCE]);
    }
            ...
            if (tep_db_num_rows($get_db_files_r) > 0) {
                while ($db_file = tep_db_fetch_array($get_db_files_r)) {
                    if ($config = $this->findConfigByFileName($db_file['file_name'])) {
                        ...
                        $update_array['run_frequency'] = $config['run_frequency'];
                        $update_array['run_time'] = $config['run_time'];
                        $update_array['job_type'] = $config['job_type'];
                        tep_db_perform(TABLE_EP_JOB, $update_array, 'update', "job_id='" . $db_file['job_id'] . "'");
                    }
                }
            }
    ...
    public function findConfigByFileName($filename)
    {
        ...
        foreach( $this->directory_config as $directory_job_config ){
            if ( fnmatch($directory_job_config['filename_pattern'],$filename,FNM_PERIOD) ) {
    ...
    public function findJobByFilename($filename)
    {
        ...
        if ( tep_db_num_rows($get_db_file_r) ) {
            ...
            return new Job($job_data);
        }
        return false;
    }
    
    public function touchImportJob($fileName, $job_state, $job_type)
    {
        clearstatcache();
        ...
        if ( !is_null($job_type) ) {
            $file_data_array['job_type'] = $job_type;
        }
        if ( !isset($file_data_array['job_type']) || empty($file_data_array['job_type']) || $file_data_array['job_type']=='auto' ) {
            ...
            if ($config) {
                ...
                $file_data_array['job_type'] = $config['job_type'];
            }
        }
        ...
        if ( tep_db_num_rows($get_db_file_r)>0 ) {
            $_db_file = tep_db_fetch_array($get_db_file_r);
