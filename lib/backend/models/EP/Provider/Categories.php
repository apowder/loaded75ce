...
class Categories extends ProviderAbstract implements ImportInterface, ExportInterface
{
    ...
    {
        $image_path = DIR_FS_CATALOG . DIR_WS_IMAGES;
        ...
        if ($found_rows == 0) {
            ...
            foreach ($main_source['columns'] as $file_column => $db_column) {
                ...
                if ($file_column == 'categories_image' && tep_not_null($data[$file_column])){
                    if (!file_exists($image_path . $data[$file_column])){
                        if (file_exists($image_path . DIRECTORY_SEPARATOR . 'import' . DIRECTORY_SEPARATOR . $data[$file_column])){
                            @copy($image_path . 'import' . DIRECTORY_SEPARATOR . $data[$file_column], $image_path . $data[$file_column]);
                        } else {
                            $data[$file_column] = '';
                            $message->info($file_primary_value . ' category image not found');
                        }
                    }
                }
