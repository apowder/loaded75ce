...
class Properties extends ProviderAbstract implements ImportInterface, ExportInterface
{
    ...
    function export(Formatter\FormatterInterface $output, $selected_fields, $filter)
    {
        ...
        $main_sql =
            "SELECT p.products_model, p.products_id, pp.properties_type, p2p.properties_id, p2p.values_id, p2p.values_flag " .
    ...
    public function importRow($data, Messages $message)
    {
        ...
        tep_db_query(
            "REPLACE INTO ".TABLE_PROPERTIES_TO_PRODUCTS." (products_id, properties_id, values_id, values_flag) ".
            "VALUES ('{$this->data['products_id']}', '{$this->data['properties_id']}', '{$this->data['values_id']}', {$this->data['values_flag']})"
        );
