...
class ElementsController extends Sceleton {
    ...
    public function actionList() {
        ...
        $elements_query_raw = "select e.elements_id, e.elements_name, count(p2e.products_id) as products_count from " . TABLE_ELEMENTS . " e left join " . TABLE_PRODUCTS_TO_ELEMENTS . " p2e on e.elements_id = p2e.elements_id where e.language_id = '" . (int)$languages_id . "' " . $search . " group by e.elements_id order by " . $orderBy;
        ...
        while ($elements = tep_db_fetch_array($elements_query)) {
            $responseList[] = array(
               '<div class="handle_cat_list state-disabled"><span class="handle"><i class="icon-hand-paper-o"></i></span>' . '<div class="cat_name">' . $elements['elements_name'] . '' . tep_draw_hidden_field('id', $elements['elements_id'], 'class="cell_identify"'). '<input class="cell_type" type="hidden" value="element"></div></div>',
    ...
    public function actionEdit() {
        ...
        $elements = tep_db_fetch_array(tep_db_query("select elements_id, elements_name, is_mandatory from " . TABLE_ELEMENTS . " where elements_id = '" . (int) $elements_id . "' and language_id = '" . (int)$languages_id . "'"));
        $eInfo = new \objectInfo($elements, false);

        echo tep_draw_form('element', FILENAME_ELEMENTS . '/save', 'elements_id=' . $eInfo->elements_id, 'post', 'onsubmit="return elementSave(' . ($eInfo->elements_id ? $eInfo->elements_id : 0) . ');"');
    ...
    public function actionConfirmdelete() {
        ...
        if ($elements_id > 0) {
            ...
            echo tep_draw_form('elements', FILENAME_ELEMENTS, \common\helpers\Output::get_all_get_params(array('eID', 'action')) . 'dID=' . $eInfo->elements_id . '&action=deleteconfirm', 'post', 'id="item_delete" onSubmit="return elementDelete();"');
    ...
    public function actionProducts() {
        ...
        $elementProducts = [];
        $query = tep_db_query("select p.products_id, p.products_status, pd.products_name from  " . TABLE_PRODUCTS_TO_ELEMENTS . " p2e, " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p2e.products_id = p.products_id and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and pd.affiliate_id = 0 and p2e.elements_id = '" . (int)$eInfo->elements_id . "' order by p2e.sort_order, pd.products_name");
...
//          $this->layout = false;
    ...
    public function actionProductSearch() {
        ...
        foreach ($categories as $category) {
            $products_query = tep_db_query("select distinct p.products_id, pd.products_name, p.products_status from " . TABLE_PRODUCTS . " p LEFT JOIN " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c ON p.products_id = p2c.products_id LEFT JOIN " . TABLE_PRODUCTS_DESCRIPTION . " pd ON p.products_id = pd.products_id where p2c.categories_id = '" . $category['id'] . "' and pd.language_id = '" . (int) $languages_id . "' and p.products_pctemplates_id = '0' and (p.products_model like '%" . tep_db_input($q) . "%' or pd.products_name like '%" . tep_db_input($q) . "%') group by p.products_id order by p.sort_order, pd.products_name limit 0, 100");
    ...
    public function actionNewProduct() {
        ...
        $query = tep_db_query("select p.products_id, pd.products_name, p.products_status from " . TABLE_PRODUCTS_DESCRIPTION . " pd," . TABLE_PRODUCTS . " p where language_id = '" . (int)$languages_id . "' and affiliate_id = 0 and p.products_id = '" . (int)$products_id . "' and p.products_id =  pd.products_id limit 1");
