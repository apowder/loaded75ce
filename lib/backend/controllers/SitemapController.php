...
class SitemapController extends Sceleton {
    ...
    public $acl = ['BOX_HEADING_SEO_CMS', 'BOX_MARKETING_SITEMAP'];
    ...
    public function actionList()
    {
        ...
        $responseList = array();
        $index_feed = tep_catalog_href_link('xmlsitemap/index');
        $content = @file_get_contents($index_feed);
        if ( $content && preg_match_all('#<sitemap>(.*?)</sitemap>#ims', $content, $feeds_match ) ) {
            $responseList[] = array(
              ...
              '<a href="'.tep_href_link('sitemap/download','file=index').'">'.TEXT_DOWNLOAD_FILE.'</a>',
            );
