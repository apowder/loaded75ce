...
class Geo_zonesController extends Sceleton {
    ...
    public function actionProductsubmit() {
        ...
        if ($sID == 0) {
			//check 
			$check = tep_db_fetch_array(tep_db_query("select association_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . (int)$zID . "' and zone_country_id = '" . (int)$zone_country_id . "' and zone_id = '" . (int)$zone_id . "'"));
			if (!$check)
