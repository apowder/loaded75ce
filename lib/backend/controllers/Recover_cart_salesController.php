  ...
  public function getDetails(&$inrec){
    ...
    \common\helpers\Translation::init('admin/recover_cart_sales');
        ...
        if ($inrec['qty'] != 0){
          ...
          $bInfo_query = tep_db_query("select cb.products_id, cb.customers_basket_quantity as qty, cb.is_giveaway, cb.final_price from " . TABLE_CUSTOMERS_BASKET . " cb where cb.customers_id = '" . (int)$inrec['cid'] . "'");
          ...
          if (is_array($coupons)){
            ...
            if (is_array($_customer_coupons) && count($_customer_coupons)){//echo '<pre>';print_r($_customer_coupons);
              ...
              if (count($__customer_coupons)){
                list($fk, $fv) = each($__customer_coupons);
      ...
      if (is_array($custid) && count($custid)) {
        ...
        foreach ($custid as $ckey=>$cid) {
          ...
          if (isset($output['coupon'][$cid]) && $use_method == 'c'){
            ...
            $data = generate_customer_gvcc($coupon_id, $inrec['email'], $output['amount'], $output['coupon_currency'], $cid, $bid[$ckey]);
