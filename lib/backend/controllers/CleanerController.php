...
class CleanerController extends Sceleton {
    ...
    public function __construct($id, $module=null){
      ...
      $cfg_group_query = tep_db_query( "select configuration_group_title, visible, configuration_group_id from " . TABLE_CONFIGURATION_GROUP . " where 1" );
      if (tep_db_num_rows($cfg_group_query)){
        while($group = tep_db_fetch_array($cfg_group_query)){
          $this->config['groups'][$group[configuration_group_id]] = ['title' => $group[configuration_group_title], 'visible' => $group[visible]];
        }
      }
      
      $cfg_group_query = tep_db_query( "select configuration_group_title, configuration_group_id from configuration_group_trash where 1" );
      if (tep_db_num_rows($cfg_group_query)){
        while($group = tep_db_fetch_array($cfg_group_query)){
          $this->config['invisible_group'][] = ['id'=>$group[configuration_group_id], 'title' => $group[configuration_group_title]];
        }
      }
    ...
    public function actionIndex() {
        global $languages_id, $language, $messageStack, $login_groups_id;

        if (file_exists(DIR_WS_LANGUAGES . $language . '/' . 'cleaner.php')) {
            include(DIR_WS_LANGUAGES . $language . '/' . 'cleaner.php');
        }
        ...
        if (tep_db_num_rows($trash_query)){
          ...
          $trash['destination'] = [];
          $dest_query = tep_db_query("select configuration_group_id, configuration_group_title from " . TABLE_CONFIGURATION_GROUP . " where 1 ");
          if (tep_db_num_rows($dest_query)){
            while($row = tep_db_fetch_array($dest_query)){
              $trash['destination'][] = ['configuration_group_id' => $row['configuration_group_id'], 'title' => $row['configuration_group_title']];
            }
  ...
  public function actionMoveBack(){
    ...
    if ($id && $group_id){
      tep_db_query("insert into " . TABLE_CONFIGURATION . " select configuration_id, configuration_title, configuration_key, configuration_value, configuration_description, '" . (int)$group_id. "', sort_order, last_modified, date_added, use_function, set_function from configuration_trash where configuration_id = '" . (int)$id . "'");
      tep_db_query("delete from configuration_trash where configuration_id = '" . (int)$id . "'");      
      $messageStack->add_session('Moved successfully', 'success');
    } else {
      $messageStack->add_session('Not Moved', 'error');
    }    
    echo 'ok';
  }
