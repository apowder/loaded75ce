...
class SubscriptionController extends Sceleton {
    ...
    public function actionIndex() {
        ...
        $subscription_status_groups_query = tep_db_query("select orders_status_groups_id, orders_status_groups_name, orders_status_groups_color from " . TABLE_ORDERS_STATUS_GROUPS . " where language_id = '" . (int)$languages_id . "' and orders_status_groups_id = '6' order by orders_status_groups_id");
        while ($subscription_status_groups = tep_db_fetch_array($subscription_status_groups_query)) {
            $status[] = [
                'name' => $subscription_status_groups['orders_status_groups_name'],
                'value' => 'group_' . $subscription_status_groups['orders_status_groups_id'],
                'selected' => '',
            ];
            $subscription_status_query = tep_db_query("select orders_status_id, orders_status_name from " . TABLE_ORDERS_STATUS . " where language_id = '" . (int)$languages_id . "' and orders_status_groups_id='" . $subscription_status_groups['orders_status_groups_id'] . "' order by orders_status_name");
            while ($subscription_status = tep_db_fetch_array($subscription_status_query)) {
                $status[] = [
                    'name' => '&nbsp;&nbsp;&nbsp;&nbsp;' . $subscription_status['orders_status_name'],
                    'value' => 'status_' . $subscription_status['orders_status_id'],
                    'selected' => '',
                ];
            }
        }
      ...
      \common\helpers\Translation::init('admin/subscription');
        ...
        while ($orders = tep_db_fetch_array($orders_query)) {
            ...
            $responseList[] = array(
                ...
                $orders['info'],
                '<div class="ord-desc-tab click_double" data-click-double="' . \Yii::$app->urlManager->createUrl(['subscription/process-subscription', 'subscription_id' => $orders['subscription_id']]) . '"><a href="' . \Yii::$app->urlManager->createUrl(['subscription/process-subscription', 'subscription_id' => $orders['subscription_id']]) . '"><span class="ord-id">' . TEXT_ORDER_NUM . $orders['subscription_id'] . (admin_id > 0 ? '&nbsp;by admin' : (\common\classes\platform::isMulti() >= 0 ? '&nbsp;' . TEXT_FROM . ' ' . \common\classes\platform::name($orders['platform_id']) : '')) . (tep_not_null($orders['payment_method']) ? ' ' . TEXT_VIA . ' ' . strip_tags($orders['payment_method']) : '') . '</span></a>' . $p_list . '</div>',
    ...
    public function actionSubscriptionActions() {
        ...
        $orders_query = tep_db_query("select o.*, s.orders_status_name from " . TABLE_ORDERS_STATUS . " s, " . TABLE_SUBSCRIPTION . " o where o.subscription_id = '" . (int) $orders_id . "'");
        $orders = tep_db_fetch_array($orders_query);
        
        if (!is_array($orders)) {
            die("Please select subscription.");
        }
        
        $oInfo = new \objectInfo($orders);
        
        echo '<div class="or_box_head">'.TEXT_ORDER_NUM . $oInfo->subscription_id . '</div>';
        ...
        echo '<div class="row_or"><div>'.TEXT_ORDER_ID . ':</div><div>'  . $oInfo->orders_id .'</div></div>';
        
        echo '<div class="btn-toolbar btn-toolbar-order"><a class="btn btn-primary btn-process-order" href="' . \Yii::$app->urlManager->createUrl(['subscription/process-subscription', 'subscription_id' => $oInfo->subscription_id]) . '">' . TEXT_PROCESS_SUBSCRIPTION_BUTTON . '</a></div>';
    }
    ...
    public function actionSubmitSubscription() {
        ...
        if (($check_status['subscription_status'] != $status) || $comments != '') {
            ...
            if ($check_status['subscription_status'] != $status) {
                ...
                if (file_exists(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/' . $check_status['payment_class'] . '.php')) {
                    Yii::$app->get('platform')->config($check_status['platform_id'])->constant_up();
                    include_once(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/' . $check_status['payment_class'] . '.php');
                }

                if (class_exists($check_status['payment_class'])) {
                    $object = new $check_status['payment_class'];
                }
                
                switch ($status) {
                    case 100003://Canceled
                        if (is_object($object)) {
...
<?php
    ...
    public function actionProcessSubscription() {
        ...
        $order = new \common\classes\subscription($oID);
            ...
            <div class="widget-content usc-box usc-box2">
                <div class="f_tab">
                        ...
                        <div class="f_row">
                            ...
                            <div class="f_td">
                                <?php echo tep_draw_textarea_field('comments', 'soft', '60', '5', '', 'class="form-control"'); ?>                             </div>
                        </div> 
                        <div class="f_row">
                            <div class="f_td"></div>
                            <div class="f_td">
...
<?php
    ...
    public function actionOrdersexport() {
        if (tep_not_null($_POST['orders'])) {
            $separator = "\t";
            $filename = 'subscription' . strftime('%Y%b%d_%H%I') . '.csv';
            ...
            while ($orders = tep_db_fetch_array($orders_query)) {
                $order = new \common\classes\subscription($orders['subscription_id']);
