...
class Pc_templatesController extends Sceleton {
    ...
    public function actionEdit() {
        ...
        echo tep_draw_form('pc_template', FILENAME_PCTEMPLATES . '/save', 'pctemplates_id=' . $tInfo->pctemplates_id, 'post', 'onsubmit="return pc_templateSave(' . ($tInfo->pctemplates_id ? $tInfo->pctemplates_id : 0) . ');"');
    ...
    public function actionConfirmdelete() {
        ...
        if ($pctemplates_id > 0) {
            ...
            echo tep_draw_form('pc_templates', FILENAME_PCTEMPLATES, \common\helpers\Output::get_all_get_params(array('tID', 'action')) . 'dID=' . $tInfo->pctemplates_id . '&action=deleteconfirm', 'post', 'id="item_delete" onSubmit="return pc_templateDelete();"');
    ...
    public function actionParts() {
        ...
        while ($elements = tep_db_fetch_array($elements_query)) {
            ...
            $r_template_elements = tep_db_query("select ppe.*,  p.products_model, p.products_price, p.products_status, p.products_price_configurator, p.products_discount_configurator, pd.products_name from " . TABLE_PRODUCTS_TO_PCTEMPLATES_TO_ELEMENTS . " ppe, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS . " p where p.products_id = ppe.products_id and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id  . "' and ppe.pctemplates_id = '" . (int)$tInfo->pctemplates_id . "' and ppe.elements_id = " . (int)$elements['elements_id'] . " and pd.affiliate_id = '0' order by ppe.sort_order");
...
//          $this->layout = false;
    ...
    public function actionProductSearch() {
        ...
        foreach ($categories as $category) {
            $products_query = tep_db_query("select distinct p.products_id, pd.products_name, p.products_status from " . TABLE_PRODUCTS_TO_ELEMENTS . " pte, " . TABLE_PRODUCTS . " p left join " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c on p.products_id = p2c.products_id left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on p.products_id = pd.products_id where pte.elements_id = '" . (int)$elements_id . "' and pte.products_id = p.products_id and p2c.categories_id = '" . (int)$category['id'] . "' and p.products_id != '" . (int) $products_id . "' and pd.language_id = '" . (int) $languages_id . "' and p.products_pctemplates_id = '0' and (p.products_model like '%" . tep_db_input($q) . "%' or pd.products_name like '%" . tep_db_input($q) . "%') group by p.products_id order by p.sort_order, pd.products_name limit 0, 100");
    ...
    public function actionNewProduct() {
        ...
        $query = tep_db_query("select p.products_id, pd.products_name, p.products_status, p.products_discount_configurator, p.products_price_configurator from " . TABLE_PRODUCTS_TO_ELEMENTS . " pte, " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where pte.elements_id = '" . (int)$elements_id . "' and pte.products_id = p.products_id and p.products_id = '" . (int)$products_id . "' and p.products_id =  pd.products_id and pd.language_id = '" . (int)$languages_id . "' and pd.affiliate_id = 0 limit 1");
