...
class TextsController extends Sceleton {
    ...
    public function actionIndex() {
        ...
        return $this->render('index', array('messages' => $messages, 'languages' => $languages));
    }
...
' . tep_draw_hidden_field('translation_key', $orders_status['translation_key'], 'class="cell_identify"') . tep_draw_hidden_field('translation_entity', $orders_status['translation_entity'], 'class="cell_type"');
    ...
    public function actionActions() {
        ...
        if (!empty($translation_key)) {
            ...
            echo '<a class="btn btn-edit btn-no-margin" href="' . \yii\helpers\Url::to(['texts/edit']) . '?' . http_build_query ($params) . '">' . IMAGE_EDIT . '</a>';            
	...
	public function actionEditSearch(){
		...
		if (Yii::$app->request->isPost){
			...
			if (tep_db_num_rows($total_query)){
				...
				return $this->redirect(\yii\helpers\Url::to(['texts/edit', 'translation_key' => $row['translation_key'], 'translation_entity' => $row['translation_entity'], 'row'=>0, 'by' => $by, 'search' => $search]) . ($sensitive? '&sensitive=1':''));
			} else{
		...
		return $this->redirect(\yii\helpers\Url::to(['texts/edit', 'row'=>0, 'by' => $by, 'search' => $search , 'empty'=> 1]) . ($sensitive? '&sensitive=1':''));
	}
    ...
    public function actionSubmit() {
        ...
        if ($gonext){
          ...
          $total_query = tep_db_query("select t.translation_key, t.translation_entity from " . TABLE_TRANSLATION. " t where t.hash in (select distinct t1.hash from " . TABLE_TRANSLATION . " t1 where t1.language_id in (" . (count( $a_info['shown_language'])? implode(',',  $a_info['shown_language']):(int)$languages_id ) . ") " . (tep_not_null($selected_entity)? "and t1.translation_entity = '" . tep_db_input($selected_entity) . "'" : '') . " " . (tep_not_null($by) && tep_not_null($search) ? " and t1.$by like " . ($sensitive?" BINARY " :'') . " '%" . tep_db_input($search) . "%'" : (tep_not_null($search) ? " and (t1.translation_entity like " . ($sensitive?" BINARY " :'') . " '%" . tep_db_input($search) . "%' or t1.translation_key like " . ($sensitive?" BINARY " :'') . " '%" . tep_db_input($search) . "%' or t1.translation_value like " . ($sensitive?" BINARY " :'') . " '%" . tep_db_input($search) . "%')" : '')). ") group by t.translation_key, t.translation_entity order by t.translation_key, t.translation_entity");
          $total_num = tep_db_num_rows($total_query);
          if ($total_num){
            $spl = new \SplFixedArray($total_num+1);
            $i =0;
    ...
    public function actionExportStart() {
        ...
        $filename  = 'Keys_' . strftime( '%Y%b%d_%H%I' ) . '.csv';
        ...
        $where = " where t.translation_value!='' and t.language_id = " . $_def_id;
        $orderBy = "translation_key";
