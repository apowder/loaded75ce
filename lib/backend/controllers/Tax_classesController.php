...
class Tax_classesController extends Sceleton  {
	...
	public function actionTax_classesactions(){
      ...
      \common\helpers\Translation::init('admin/tax_classes');
		...
		if ($tax_classes_id){
			...
			echo '</div>';
			$contents[] = array('align' => 'center', 'text' => '<input type="button" value="' . IMAGE_EDIT . '" class="btn btn-primary" onclick="taxEdit('.$tax_classes_id.')">&nbsp;'.
			'<input type="button" value="' . IMAGE_DELETE . '" class="btn btn-primary" onclick="taxDelete('.$tax_classes_id.')">');

			$contents[] = array('text' => '<br>' . TEXT_INFO_DATE_ADDED . ' ' . \common\helpers\Date::date_short($tcInfo->date_added));
			$contents[] = array('text' => '' . TEXT_INFO_LAST_MODIFIED . ' ' . \common\helpers\Date::date_short($tcInfo->last_modified));
	...
	public function actionEdit(){
	  ...
	  if($tax_classes_id){
		...
		echo tep_draw_form('classes', FILENAME_TAX_CLASSES, 'page=' . $HTTP_GET_VARS['page'] . '&tID=' . $tcInfo->tax_class_id . '&action=save');
		echo '<div class="row_or_wrapp">';
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . '<input type="button" value="' . IMAGE_UPDATE . '" class="btn btn-primary" onclick="taxSave('.($tcInfo->tax_class_id?$tcInfo->tax_class_id:0).')">
