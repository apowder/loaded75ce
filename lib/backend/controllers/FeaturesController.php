...
class FeaturesController extends Sceleton {
  ...
  public function actionIndex() {
    global $messageStack, $languages_id, $language;
  ...
  public function actionView() {
    ...
    if ($fInfo->features_id > 0) {
      ...
      $messageStack->add_session('Wrong feature ID!', 'error');
  ...
  public function actionInstall() {
    ...
    if ($fInfo->features_id > 0) {
      if (!$fInfo->feature_enabled) {
        ...
        if ($response == 'OK') {
          $messageStack->add_session($fInfo->features_title . ' has been successfully installed!', 'success');
        } else {
          $messageStack->add_session($response, 'error');
        }
        return $this->redirect(Yii::$app->urlManager->createUrl(['features/view', 'fID' => $features_id]));
      } else {
        $messageStack->add_session($fInfo->features_title . ' is already installed!', 'error');
        return $this->redirect(Yii::$app->urlManager->createUrl(['features/view', 'fID' => $features_id]));
      }
    } else {
      $messageStack->add_session('Wrong feature ID!', 'error');
  ...
  public function actionUninstall() {
    ...
    if ($fInfo->features_id > 0) {
      if ($fInfo->feature_enabled) {
        ...
        if ($response == 'OK') {
          $messageStack->add_session($fInfo->features_title . ' has been successfully uninstalled!', 'success');
        } else {
          $messageStack->add_session($response, 'error');
        }
        return $this->redirect(Yii::$app->urlManager->createUrl(['features/view', 'fID' => $features_id]));
      } else {
        $messageStack->add_session($fInfo->features_title . ' is already uninstalled!', 'error');
        return $this->redirect(Yii::$app->urlManager->createUrl(['features/view', 'fID' => $features_id]));
      }
    } else {
      $messageStack->add_session('Wrong feature ID!', 'error');
