<div class="is-virtual-btn">{$smarty.const.TEXT_IS_VIRTUAL} <input type="checkbox" value="1" name="is_virtual" class="is_virt_on_off"{if $pInfo->is_virtual == 1} checked{/if} /></div>
<div class="is-virtual-upload"{if $pInfo->is_virtual == 0} style="display: none;"{/if}>
  ...
  <!--<a href="" class="btn btn-primary">{$smarty.const.TEXT_UPLOAD_PRODUCT}</a>!-->
</div>
<div class="is-virtual"{if $pInfo->is_virtual == 1} style="display: none;"{/if}>
  ...
  <div class="cartoon_wrapper">
    {if \common\helpers\Acl::checkExtension('PackUnits', 'productBlock')}
      {\common\extensions\PackUnits\PackUnits::productBlock($pInfo)}
    {else}
...
<div class="popup-box-wrap-page" style="display: none" id="pack-price-management">
            ...
            <div class="tab-prices">
              ...
              <div class="tabbable tabbable-custom">
                <ul class="nav nav-tabs">
                  <li class="active"><a href="#tab_pack_unit-0" data-toggle="tab"><span>{$smarty.const.TEXT_MAIN}</span></a></li>
                  {foreach $app->controller->view->groups as $groups_id => $group}
                    <li>
                      <a href="#tab_pack_unit-{$groups_id}" data-toggle="tab">
                ...
                <div class="tab-content">
                  <div class="tab-pane active" id="tab_pack_unit-0">
                    {/if}
                    <div style="padding-right: 65px">
                      <div class="t-row js_pack_unit_group_fullprice" data-group_discount="0">
                        <div class="t-col-3">
                          <label><input type="radio" name="ifpopt_pack_unit_0" value="-2">Use base product price</label>
                          <label><input type="radio" name="ifpopt_pack_unit_0" value="1">Set price</label>
                        </div>
                        <div class="t-col-3">
                          <label for="">{$smarty.const.TEXT_NET_PRICE}</label>
                          <div class=""><input type="text" name="pack_unit_full_prices[0]" value="{$pInfo->products_price_pack_unit}" class="form-control form-control-small js_inventory_price_input" onkeyup="updateGross()"></div>
                        </div>
                        <div class="t-col-3">
                          <label for="">{$smarty.const.TEXT_GROSS_PRICE}</label>
                          <div class=""><input type="text" name="pack_unit_full_gross_prices[0]" value="" class="form-control form-control-small" onkeyup="updateNet()"></div>
                        </div>
                      </div>
                      <div class="t-row">
                        <div class="t-col-3">
                        ...
                        <div class="t-col-3">
                          <input type="checkbox" value="1" name="full_qty_discount_status_pack_unit" class="check_qty_discount_pack_unit" {if {$app->controller->view->qty_discounts_pack_unit|@count} > 0} checked="checked" {/if} />
                        </div>
                    ...
                    <div class="qty-discount-wrap">
                      ...
                      <div class="wrap-quant-discount-full-price-pack_unit">
                        {if {$app->controller->view->qty_discounts_pack_unit[0]|@count} > 0}
                          {foreach $app->controller->view->qty_discounts_pack_unit[0] as $qty => $price}
                            <div class="quant-discount-line after div_qty_discount_pack_unit" {if not {$app->controller->view->qty_discounts_pack_unit|@count} > 0} style="display:none;" {/if}>
                              <div>
                                <label>{$smarty.const.TEXT_PRODUCTS_QUANTITY_INFO}</label><input type="text" class="form-control" name="inventory_discount_full_qty_pack_unit[]" value="{$qty}">
                              </div>
                              <div>
                                <label>{$smarty.const.TEXT_NET}</label><input type="text" class="form-control" name="inventory_discount_full_price_pack_unit[]" value="{$price}" onkeyup="updateGross()">
                              </div>
                              <div>
                                <label>{$smarty.const.TEXT_GROSS}</label><input type="text" class="form-control" name="inventory_discount_full_gross_price_pack_unit[]" value="" onkeyup="updateNet()">
                              </div>
                      ...
                      <div class="quant-discount-btn div_qty_discount_pack_unit" {if not {$app->controller->view->qty_discounts_pack_unit|@count} > 0} style="display:none;" {/if}>
                              <span class="btn btn-add-more-inv-qty-full-price" data-uprid="pack_unit" data-aprid="pack_unit">{$smarty.const.TEXT_ADD_MORE}/span>
                      </div>
                    </div>
                  ...
                  {foreach $app->controller->view->groups as $groups_id => $group}
                    ...
                    <div class="tab-pane" id="tab_pack_unit-{$groups_id}">

                      <div class="t-row js_pack_unit_group_fullprice" data-uprid="pack_unit" data-group_id="{$groups_id}" data-group_discount="{$group['groups_discount']}">
                        <div class="t-col-3">
                          <label><input type="radio" name="ifpopt_pack_unit_{$groups_id}" value="-2">{$smarty.const.TEXT_PRICE_SWITCH_MAIN_PRICE}</label>
                          <label><input type="radio" name="ifpopt_pack_unit_{$groups_id}" value="1">{sprintf($smarty.const.TEXT_PRICE_SWITCH_OWN_PRICE, $group['groups_name'])}</label>
                          <label><input type="radio" name="ifpopt_pack_unit_{$groups_id}" value="-1">{sprintf($smarty.const.TEXT_PRICE_SWITCH_DISABLE, $group['groups_name'])}</label>
                        </div>
                        <div class="js_inventory_price_block">
                          <div class="t-col-3">
                            <label for="">{$smarty.const.TEXT_NET_PRICE}</label>
                            <div class=""><input type="text" name="pack_unit_full_prices[{$groups_id}]" value="{$app->controller->view->products_group_price_pack_unit[$groups_id]}" class="form-control form-control-small js_inventory_price_input" onkeyup="updateGross()"></div>
                          </div>
                          <div class="t-col-3">
                            <label for="">{$smarty.const.TEXT_GROSS_PRICE}</label>
                            <div class=""><input type="text" name="pack_unit_full_gross_prices[{$groups_id}]" value="" class="form-control form-control-small" onkeyup="updateNet()"></div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="qty-discount-wrap">
                        ...
                        <div class="wrap-quant-discount-full-price-pack_unit-{$groups_id}">
                          {if {$app->controller->view->qty_discounts_pack_unit[$groups_id]|@count} > 0}
                            {foreach $app->controller->view->qty_discounts_pack_unit[$groups_id] as $qty => $price}
                              <div class="quant-discount-line after div_qty_discount_pack_unit" {if not {$app->controller->view->qty_discounts_pack_unit|@count} > 0} style="display:none;" {/if}>
                                <div>
                                  <label>{$smarty.const.TEXT_PRODUCTS_QUANTITY_INFO}</label><input type="text" class="form-control" name="inventory_discount_full_qty_pack_unit_{$groups_id}[]" value="{$qty}">
                                </div>
                                <div>
                                  <label>{$smarty.const.TEXT_NET}</label><input type="text" class="form-control" name="inventory_discount_full_price_pack_unit_{$groups_id}[]" value="{$price}" onkeyup="updateGross()">
                                </div>
                                <div>
                                  <label>{$smarty.const.TEXT_GROSS}</label><input type="text" class="form-control" name="inventory_discount_full_gross_price_pack_unit_{$groups_id}[]" value="" onkeyup="updateNet()">
                                </div>
                                <span class="rem-quan-line"></span>
                              </div>
                            {/foreach}
                        ...
                        <div class="quant-discount-btn div_qty_discount_pack_unit" {if not {$app->controller->view->qty_discounts_pack_unit|@count} > 0} style="display:none;" {/if}>
                          <span class="btn btn-add-more-inv-qty-full-price" data-groups_id="{$groups_id}" data-uprid="pack_unit" data-aprid="pack_unit">{$smarty.const.TEXT_ADD_MORE}</span>
                        </div>
                      </div>
                  ...
                  <div class="tab-pane active" id="tab_packaging-0">
                    ...
                    <div style="padding-right: 65px">
                      ...
                      <div class="t-row">
                        ...
                        <div class="t-col-3">
                          <input type="checkbox" value="1" name="full_qty_discount_status_packaging" class="check_qty_discount_packaging" {if {$app->controller->view->qty_discounts_packaging|@count} > 0} checked="checked" {/if} />
                        </div>
                        <div class="t-col-3"></div>
                      </div>
                    </div>
                    ...
                    <div class="qty-discount-wrap">
                      ...
                      <div class="wrap-quant-discount-full-price-packaging">
                        {if {$app->controller->view->qty_discounts_packaging[0]|@count} > 0}
                      ...
                      <div class="quant-discount-btn div_qty_discount_packaging" {if not {$app->controller->view->qty_discounts_packaging|@count} > 0} style="display:none;" {/if}>
                    ...
                    <div class="tab-pane" id="tab_packaging-{$groups_id}">
                      ...
                      <div class="qty-discount-wrap">
                        ...
                        <div class="wrap-quant-discount-full-price-packaging-{$groups_id}">
                          {if {$app->controller->view->qty_discounts_packaging[$groups_id]|@count} > 0}
...
<script type="text/javascript">
  function editPackPrice() {
    var popup = $('#pack-price-management');
    popup.show();
    ...
    $('.pop-up-close-page, .btn-cancel', popup).off('click').on('click', function(){
      ...
      updateGross();
      //update_inventory_prices($('#full_add_price').val());
      popup.hide();
  ...
  function editPackagingPrice() {
    var popup = $('#packaging-price-management');
    popup.show();
    $('#content, .content-container').css({ 'position': 'relative', 'z-index': '100'});
    $('.w-or-prev-next > .tabbable').css({ 'z-index': '5'});
