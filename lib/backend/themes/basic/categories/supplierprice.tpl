...
{if {$app->controller->view->suppliers|@count} > 0}
    {foreach $app->controller->view->suppliers as $suppliers_id => $supplier}
        <tr>
            ...
            <td class="label_value"><a href="javascript:void(0)" class="btn btn-primary" onclick="selectSupplierPrice({$suppliers_id}, {$supplier['groups_id']}); return cancelStatement();">{$smarty.const.IMAGE_SELECT}</a></td>
        </tr>
    {/foreach}
{/if}
    </table>
