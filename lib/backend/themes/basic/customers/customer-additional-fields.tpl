...
<form name="customer_edit" id="customers_edit" onSubmit="return check_form();">
    ...
    <div class="box-wrap">
        ...
        <div class="widget box box-no-shadow">
            <div class="widget-header"><h4>{$smarty.const.CUSTOMER_DETAILS}</h4></div>
            ...
            <div class="widget-content">
                        ...
                        <div class="widget box box-no-shadow">
                            ...
                            <div class="widget-content">
                                    ...
                                    {foreach $addresses as $address}
                                        <div>
                                            <label class="address-fields{if $customer.customers_default_address_id == $address.id} default-address{/if}">
                                                ...
                                                <b>{$smarty.const.TABLE_HEADING_EMAIL}:</b> {if $customer.customers_email_address}{$customer.customers_email_address}{else}____{/if},<br>
                                ...
                                <div class="w-line-row w-line-row-2">
                                    ...
                                    <div>
                                        <div class="wl-td">
                                            <label>{$smarty.const.TABLE_HEADING_EMAIL}</label>
                                    ...
                                    {foreach $addresses as $address}
                                        <div>
                                            <label class="address-fields{if $customer.customers_default_address_id == $address.id} default-address2{/if}">
                                                ...
                                                <b>{$smarty.const.TEXT_INFO_FULLNAME}</b> {$customer.customers_firstname} {$customer.customers_lastname},<br>
                                                <b>{$smarty.const.ENTRY_TELEPHONE_NUMBER}:</b> {if $customer.customers_telephone}{$customer.customers_telephone}{else}____{/if},<br>
                                                <b>{$smarty.const.TABLE_HEADING_EMAIL}:</b> {if $customer.customers_email_address}{$customer.customers_email_address}{else}____{/if},<br>
                                                <b>{$smarty.const.CATEGORY_ADDRESS}:</b> {if $address.address}{$address.address}{else}____{/if}
                ...
                <div class="w-line-row w-line-row-2 additional-fields">
                    <div>
                        <div class="wl-td">
        ...
        <div class="widget box box-no-shadow">
            <div class="widget-header"><h4>{$smarty.const.TEXT_DISCOUNT}</h4></div>
            <div class="widget-content additional-fields">
        ...
        <div class="widget box box-no-shadow">
            <div class="widget-header"><h4>{$smarty.const.BANK_ACCOUNT_DETAILS}</h4></div>
            <div class="widget-content additional-fields">
        ...
        <div class="widget box box-no-shadow">
            <div class="widget-header"><h4>{$smarty.const.TRADE_REFERENCES}</h4></div>
            <div class="widget-content additional-fields">
        ...
        <div class="widget box box-no-shadow">
            <div class="widget-header"><h4>{$smarty.const.TEXT_DECLARATION}</h4></div>
            <div class="widget-content additional-fields">
        ...
        {*foreach $additionalFields as $group}
            <div class="widget box box-no-shadow">
                <div class="widget-header"><h4>{$group.name}</h4></div>
                <div class="widget-content additional-fields">
                    <div class="w-line-row w-line-row-2 additional-fields">
                        {foreach $group.child as $field}
                            <div{if $group.name == 'Discount'} class="checkboxes"{/if}>
                                <div class="wl-td">
                                    <label>{$field.name}</label>
                                    {if $field.field_type == 'checkbox'}
                                        <input class="form-checkbox" name="field[{$field.id}]" value="1"{if $field.value} checked{/if} type="checkbox">
                                    {else}
                                        <input class="form-control" name="field[{$field.id}]" value="{$field.value}" type="text">
                                    {/if}
                                </div>
                            </div>
                        {/foreach}
                ...
                $('.select-field').each(function(){
                    ...
                    if (!check){
                        if (!value){
                            ...
                            var span = $('input[type="radio"]:checked', this).closest('label').find('span');
                            hidden.val(span.html())
                        } else {
                            $('input[type="radio"]:last', this).prop( "checked", true );
                            $('input[type="text"]', this).val(value)
                        }
                    }
                    $('input[type="radio"]', this).on('change', function(){
                        ...
                        if (span.hasClass('val')){
                            ...
                            hidden.val($('input', span).val())
                        }
                    });
                    $('input[type="text"]', this).on('focus', function(){
