...
function addProductUpdate(id){
    //$("#order_management").hide();
    $(window).scrollTop(0);
    $.post("{$app->urlManager->createUrl('subscription/addproductprocess')}", $('form[name=add_product]').serialize()+'&oID='+id, function(data, status){
        if (status == "success") {
            $.post("{$app->urlManager->createUrl('subscription/order-edit')}", {
                'orders_id': id,
            }, function (data, status) {
                if (status == "success") {  
                    $("#order_management_data").html(data);
