      ...
      <div class="after">
        <div class="attr-box attr-box-1">
          <div class="widget widget-attr-box box box-no-shadow" style="margin-bottom: 0;">
            ...
            <div class="widget-content">
              <select id="pctemplate-{$elements_id}-search-products" size="25" style="width: 100%; height: 100%; border: none;" ondblclick="addSelectedPoduct{$elements_id}()">
              </select>
            </div>
          </div>
        </div>
  ...
  function addSelectedPoduct{$elements_id}() {
    $( 'select#pctemplate-{$elements_id}-search-products option:selected' ).each(function() {
      ...
      if ( $('input[name="pctemplate_{$elements_id}_products_id[]"][value="' + products_id + '"]').length ) {
        ...
        $.post("{Yii::$app->urlManager->createUrl('pc_templates/new-product')}", { 'elements_id': {$elements_id}, 'products_id': products_id }, function(data, status) {
          if (status == "success") {
