...
<div>
    ...
    <table id="tblDirectorySetting" data-row_count="{count($directoryConfigs)}" width="100%" cellpadding="1" cellspacing="1">
        <thead>
        <tr>
            <td>{$smarty.const.TEXT_FILE_MASK}</td>
        ...
        <tr class="js_row js_row_{$index}">
            <td><span class="ui-front ac-holder">{Html::textInput('directory_config['|cat:$index|cat:'][filename_pattern]', $config['filename_pattern'], ['class' => 'form-control js_ac-ondemand', 'data-ac-source'=>$directoryFilesSuggestSource])}</span></td>
            ...
            <td>{Html::dropDownList('directory_config['|cat:$index|cat:'][job_type]', $config['job_type'], $providersList['items'], ['class' => 'form-control'])}</td>
            ...
            <td><button class="btn btn-remove js-directory-config-remove">{$smarty.const.TEXT_REMOVE}</button></td>
        </tr>
        {/foreach}
        </tbody>
        <tfoot style="display: none">
        <tr class="js_row js_row__cnt_">
            <td><span class="ui-front ac-holder">{Html::textInput('directory_config_new_[filename_pattern]', '', ['class' => 'form-control js_ac-ondemand', 'data-ac-source'=>$directoryFilesSuggestSource])}</span></td>
            <td>{Html::dropDownList('directory_config_new_[file_format]', $formatReaders['selection'], $formatReaders['items'], ['class' => 'form-control'])}</td>
            <td>{Html::dropDownList('directory_config_new_[job_type]', $providersList['selection'], $providersList['items'], ['class' => 'form-control'])}</td>
