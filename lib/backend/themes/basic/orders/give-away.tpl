...
{/if}                
			...
			<div class="ga_wrapper">
			  ...
			  {foreach $products as $product name=foo}
				...
				{/if}
				<div class="{if $nopopup eq false}ga_column{/if} after">
				  ...
				  <div class="ga_ovr">
					{if $product.active && !$nopopup}
					<form name="cart_quantity" action="{\yii\helpers\Url::to(['orders/addproduct', 'orders_id' => $oID])}" method="post" id="product-form" class="frmGaProduct">
						{tep_draw_hidden_field('action', 'add_giveaway')}
						{tep_draw_hidden_field('product_id', {$product['products_id']})}
					{/if}
					<div class="ga_name">
					...
					<div class="ga_price after">
					  {if $product.active && !$nopopup}
						...
						<span class="checkBoxWrap"><input type="checkbox" name="giveaway_switch[{$product.ga_idx}]" value="1" {if $product.single_checkbox}checked{/if} onClick="this.form.submit();"></span><br>
					  {/if}
					  <br/>
