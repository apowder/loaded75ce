          ...
          {function name=categoriesTree}
            {foreach $categories as $item}
              {if $item.parent_id == $parent}
                <li data-type-id="{$item.categories_id}" data-type="categories">
                  <div class="item-handle"><div class="item-handle-move"></div><div class="item-close"></div><div class="searchable">{$item.categories_name}</div><div class="edit"></div><div class="remove"></div>
                  ...
                  <ul>
                    {call categoriesTree parent=$item.categories_id}
                  </ul>
                </li>
              {/if}
            {/foreach}
  ...
  <div class="menu-right-list">
    ...
    <div class="select-menu">
      ...
      <form id="select-menu" action="{$action_url_select_menu}" method="get">
        ...
        {/if}
        {if $current_menu.menu_name != '' && $current_menu.menu_name != 'Categories' && $current_menu.menu_name != 'Header menu'}
        <div class="edit"></div>
    ...
    <div class="drop-list connect-list">
      ...
      {function name=menuTree}
          {foreach $menu as $item}
            {if $item.parent_id == $parent}
              <li{if isset($item.id)} data-id="{$item.id}"{/if} data-type-id="{$item.link_id}" data-type="{$item.link_type}" {if isset($item.new_category)} class="new-category"{/if}>
...
<script>
  ...
  $(document).ready(function() {
    ...
    var categories_tree_html = $('.categories-tree').html()
    ...
    $('.btn-save-menu-name').on('click', function(){
      $.get('{tep_href_link('menus/save-name')}', {
        name: $('input', select_menu).val(),
        id: $('select', select_menu).val()
      }, function(){
        $('.edit-name', select_menu).hide();
        $('.menu-name', select_menu).show();
        $('.edit', select_menu).removeClass('active');
        $('.menu-name', select_menu).text($('input', select_menu).val());
        $('select option:selected', select_menu).text($('input', select_menu).val())
      }, 'json')
    });
    ...
    var sort = {
      ...
      update: function(d, e){
        ...
        if (e.item.data('type-id') == '999999999'){
          $('> ul', e.item).html(categories_tree_html);
