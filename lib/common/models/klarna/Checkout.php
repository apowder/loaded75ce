...
spl_autoload_register(function($class){
    ...
    $class = str_replace("common\models\Klarna_Checkout_", "", $class);
    //if (strpos($class, 'Connector') !== false) {var_dump($class);die;}
    if (strpos($class, 'HTTP') !== false){
        $class = str_replace("HTTP_", "", $class);
        require_once KLARNA_CHECKOUT_DIR . '/HTTP/' . $class . '.php';
    } else {
        require_once KLARNA_CHECKOUT_DIR . '/' . $class . '.php';
    }
});
/*
