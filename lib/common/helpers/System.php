...
class System {
    ...
    public static function setcookie($name, $value = '', $expire = 0, $path = '/', $domain = '', $secure = 0) {
        setcookie($name, $value, $expire, $path, (tep_not_null($domain) ? $domain : ''), $secure);
    }

    public static function exploding_cookie($cookie) {
        $params = array();
     // utmgclid - unique id grom google adwords
     // utmccn - info about adwords or organic search
     // utmcmd - info about adwords or organic search
     // utmcsr - referal
     // utmcct - 
     // utmctr - keyword        
        $search = array('utmgclid', 'utmccn', 'utmcmd', 'utmcsr', 'utmcct', 'utmctr');
        $list = explode("|", $cookie);
        if (sizeof($list) > 0) {
            foreach ($list as $val) {
                for ($i = 0; $i < sizeof($search); $i++) {
                    $pos = strpos($val, $search[$i]);
                    if ($pos !== false) {
                        $params[$search[$i]] = substr($val, $pos + strlen($search[$i]) + 1);
                    }
                }
            }
        }
    ...
    public static function ga_detection($order_id = 0) {
        global $_COOKIE, $customer_id, $_SESSION;

        $check = tep_db_fetch_array(tep_db_query("select count(*) as cnt from " . TABLE_GA . " where customers_id = '" . (int) $customer_id . "' and orders_id='" . (int) $order_id . "'"));
        if ($check['cnt'] == 0) {
            $params = \common\helpers\System::exploding_cookie($_COOKIE['__utmz']);
			$ua = new \common\models\UserAgentParser();
			$agent = serialize($ua->parse_user_agent());
            $online = tep_db_fetch_array(tep_db_query("select * from " . TABLE_WHOS_ONLINE . " where session_id = '" . tep_session_id() . "'"));
            $sql_array = array('customers_id' => (int) $customer_id,
                'orders_id' => (int) $order_id,
                'utmgclid' => @$params['utmgclid'],
                'utmccn' => @$params['utmccn'],
                'utmcmd' => @$params['utmcmd'],
                'utmctr' => @$params['utmctr'],
                'utmcsr' => @$params['utmcsr'],
                'user_agent' => $agent,
                'ip_address' => self::get_ip_address(),
                'last_page_url' => @$online['last_page_url'],
                'resolution' => (isset($_SESSION['resolution']) ? $_SESSION['resolution'] : '')
            );
            
            tep_db_perform(TABLE_GA, $sql_array);
            unset($_SESSION['resolution']);
        }

        if (class_exists('\common\models\Google') && $order_id) {
            $google = new \common\models\Google();
            $google->checkOrderPosition($order_id);
        }
    }
