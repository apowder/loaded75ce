...
//      if (defined(constant($module_key)) && tep_not_null(constant($module_key))) {
      if (tep_not_null(constant($module_key))) {
        ...
        } elseif ( (tep_not_null($module)) && (in_array(substr($module, 0, strpos($module, '_')) . '.' . substr($PHP_SELF, (strrpos($PHP_SELF, '.')+1)), $this->modules)) ) {
          $this->selected_module = substr($module, 0, strpos($module, '_'));
          $include_modules[] = array('class' => substr($module, 0, strpos($module, '_')), 'file' => substr($module, 0, strpos($module, '_')) . '.' . substr($PHP_SELF, (strrpos($PHP_SELF, '.')+1)));
        } else {
          reset($this->modules);
...
// EOF: WebMakers.com Added: Downloads Controller
        ...
        for ($i=0, $n=sizeof($include_modules); $i<$n; $i++) {
		  ...
		  if (\frontend\design\Info::isTotallyAdmin()){
			  if ( !is_file(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/' . $include_modules[$i]['file']) ) continue;
			  include(DIR_FS_CATALOG . DIR_WS_MODULES . 'payment/' . $include_modules[$i]['file']);			  			  
		  } else {
			  if ( !is_file(DIR_WS_MODULES . 'payment/' . $include_modules[$i]['file']) ) continue;
...
// $HTTP_POST_VARS['payment'] value which will be empty (no radio button selection possible)
        if ( (\common\helpers\Modules::count_payment_modules() == 1) && (!isset($GLOBALS[$payment]) || (isset($GLOBALS[$payment]) && !is_object($GLOBALS[$payment]))) ) {
            if (!isset($GLOBALS[substr($payment, 0, strpos($payment, '_'))]) || (isset($GLOBALS[substr($payment, 0, strpos($payment, '_'))]) && !is_object($GLOBALS[substr($payment, 0, strpos($payment, '_'))]))) {
...
*/   
    ...
    function javascript_validation() {
      ...
      if (is_array($this->modules)) {
        ...
        while (list(, $value) = each($this->modules)) {
          $class = substr($value, 0, strrpos($value, '.'));
          if ( !is_object($GLOBALS[$class]) ) continue;
...
//  ICW CREDIT CLASS Gift Voucher System Line below amended
    ...
    function selection( $opc = false, $onlyOnline = false ) {
        global $order;
        $selection_array = array();

      if (is_array($this->modules)) {
        $haveSubscription = $order->haveSubscription();
        reset($this->modules);
        while (list(, $value) = each($this->modules)) {
          $class = substr($value, 0, strrpos($value, '.'));
          if ( !is_object($GLOBALS[$class]) ) continue;
          if ($haveSubscription) {
            if (!$GLOBALS[$class]->haveSubscription()) {
              continue;
            }
          }
          if ($onlyOnline && !$GLOBALS[$class]->isOnline()) {
              continue;
          }
          if ( $opc ) {
...
function check_credit_covers() {
    ...
    function isOnline() {
      if (is_array($this->modules)) {
        if (is_object($GLOBALS[$this->selected_module]) && ($GLOBALS[$this->selected_module]->enabled) ) {
          return $GLOBALS[$this->selected_module]->isOnline();
        }
      }
    }
    ...
    function before_subscription($id) {
      if (is_array($this->modules)) {
        if (is_object($GLOBALS[$this->selected_module]) && ($GLOBALS[$this->selected_module]->enabled) ) {
          return $GLOBALS[$this->selected_module]->before_subscription($id);
        }
      }
    }
    ...
    function get_error() {  
      if (is_array($this->modules)) {
        if (is_object($GLOBALS[$this->selected_module]) && ($GLOBALS[$this->selected_module]->enabled) ) {
          global $messageStack;
          $_error = $GLOBALS[$this->selected_module]->get_error();
