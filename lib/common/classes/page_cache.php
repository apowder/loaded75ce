...
class page_cache {
	...
	var $cache_dir, $cache_filename, $cache_param, $cache_file, $cache_compress, $cache_language, $cache_currency, $customer_sid;
  var $all_uncached_data_array = array();
...
###################################################################################*/
	function __construct($all_uncached_data_arr=array(), $cache_time = PAGE_CACHE_LIFETIME, $debug_switch = PAGE_CACHE_DEBUG_MODE) {
		global $SID;
    global $affiliate_ref;
    $this->all_uncached_data_array=$all_uncached_data_arr;
		...
		$this->cache_param = preg_replace("/([&]|)".preg_quote(tep_session_name(),"/")."=[^&]+/", "", $this->cache_param);
...
// }}
		...
		$this->cache_language = '_' . ($_SESSION['language']!=""?$_SESSION['language']:"english");
		$this->debug_output['cache_language'] = $this->cache_language;
		# Set the cache file currency and store it
		$this->cache_currency = '_' . ($_SESSION['currency']!=""?$_SESSION['currency']:"GBP");
		...
		if (PAGE_CACHE_DISABLE_PARAMETERS == 'false') {
			...
			$this->cache_file = $this->cache_dir.$this->cache_filename.($this->cache_param?'_'.$this->cache_param:'').$this->cache_language.$this->cache_currency.($affiliate_ref>0?"_".$affiliate_ref:"").".cache";
	  } else {
      # Parameters ARE disabled so DO NOT include them in the cache_file naming
      $this->cache_file = $this->cache_dir.$this->cache_filename.$this->cache_language.$this->cache_currency.($affiliate_ref>0?"_".$affiliate_ref:"").".cache";
    }
    ...
    */
    if (is_array($this->all_uncached_data_array) && count($this->all_uncached_data_array))
    {
      foreach($this->all_uncached_data_array as $uncached_name=> $uncached_item_data)
      {
        foreach ($uncached_item_data as $uncached_item=>$uncached_data)
        {
            $this->{$uncached_item} = $this->compress_buffer( $uncached_data );
            $this->debug_output[$uncached_item] = ( is_string($this->{$uncached_item})  ? true : false );
        }
      }
    }
...
//{{
        ...
        foreach($this->all_uncached_data_array as $uncached_name=> $uncached_item_data)
        {
          foreach ($uncached_item_data as $uncached_item=>$uncached_data)
          {
            $mixed_search[]=$uncached_name;
            $mixed_replace[]=$this->{$uncached_item};
          }
        }
//}}
        if ( !tep_not_null($this->customer_sid) )
        {
          $mixed_search[]="?(".tep_session_name().")";
          $mixed_search[]="&(".tep_session_name().")";
          $mixed_replace[]=$this->customer_sid;
          $mixed_replace[]=$this->customer_sid;
          # They have cookies enabled...let's remove the osCsid from the URL's since it is being pulled from cookie
          echo str_replace($mixed_search, $mixed_replace, file_get_contents($this->cache_file) );
        } else {
          $mixed_search[]="(".tep_session_name().")";
          $mixed_replace[]=$this->customer_sid;
...
###################################################################################*/
		...
		if ( !tep_not_null($this->customer_sid) ) 
    {
      ...
      $mixed_replace[]=$this->customer_sid;
      $mixed_replace[]=$this->customer_sid;
			echo str_replace($mixed_search, $mixed_replace, $buffer );
		} else {
      $mixed_search[]="(".tep_session_name().")";
      $mixed_replace[]=$this->customer_sid;
			echo str_replace($mixed_search, $mixed_replace, $buffer);
