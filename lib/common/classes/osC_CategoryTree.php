    ...
    function buildBranch($parent_id, $level = 0, $cid = 0, $cid2 = 0) {
      ...
      if (isset($this->data[$parent_id])) {
        ...
        foreach ($this->data[$parent_id] as $category_id => $category) {
          ...
          if (is_array($HTTP_GET_VARS['c'])) {
            $categories_array = $HTTP_GET_VARS['c'];
          } else {
            $categories_array = array();
          }
          if (in_array($category['c_id_s'], $categories_array)) {
            unset($categories_array[array_search($category['c_id_s'], $categories_array)]);
            $result .= str_repeat($this->spacer_string, $this->spacer_multiplier * $level) . '<a href="' . tep_href_link('catalog', 'cPath=' . $HTTP_GET_VARS['cPath'] . (count($categories_array) > 0 ? '&c[]=' . implode('&c[]=', $categories_array) : '')) . '" class="c_heck selected_check"></a>';
          } else {
            $categories_array[] = $category['c_id_s'];
            $result .= str_repeat($this->spacer_string, $this->spacer_multiplier * $level) . '<a href="' . tep_href_link('catalog', 'cPath=' . $HTTP_GET_VARS['cPath'] . (count($categories_array) > 0 ? '&c[]=' . implode('&c[]=', $categories_array) : '')) . '" class="c_heck"></a>';
          }
