        ...
        if ( count($fields)==0 || array_key_exists('image_data', $fields) ) {
            if ( !empty($this->hash_file_name) ) {
                ...
                if ( is_file($imageFilename) ){
                    ...
                    $this->image_source_url = \Yii::$app->get('platform')->config()->getCatalogBaseUrl().Images::getWSCatalogImagesPath(false).'products/'.$this->parentObject->products_id.'/'.$this->products_images_id.'/'.$this->hash_file_name;
                }
                foreach(Images::getImageTypes() as $image_types){
                    ...
                    if (file_exists($image_location . $imageName)) {
                        ...
                        $this->image_sources[] = [
                            ...
                            'url' => \Yii::$app->get('platform')->config()->getCatalogBaseUrl().Images::getWSCatalogImagesPath(false).$partial_file_name,
                        ];
                    }
                }
            }
...
//        }
    ...
    {
        if ( !empty($this->hash_file_name) && $this->generateImageThumbnails ) {
            $imageFilename = Images::getFSCatalogImagesPath() . 'products/' . $this->parentObject->products_id . '/' . $this->products_images_id . '/' . $this->hash_file_name;

            if ( is_file($imageFilename) /*&& !empty($this->image_data)*/ ) {
                try {

                    /*FileHelper::createDirectory(dirname($imageFilename), 0777);
                    file_put_contents( $imageFilename, $this->image_data );
                    chmod($imageFilename, 0666);*/
                    static $Images;
                    if ( !is_object($Images) ) {
                        $Images = new Images();
                    }
                    $lang = '';
                    if ( $this->language_id>0 ) $lang = \common\classes\language::get_code($this->language_id);
                    $file_name = $this->file_name;
                    $Images->createImages($this->parentObject->products_id, $this->products_images_id, $this->hash_file_name, $file_name, $lang);//$orig_file
