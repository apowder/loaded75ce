...
class Categories extends Widget
{
  ...
  public function run()
  {
    global $languages_id, $current_category_id, $HTTP_SESSION_VARS;
    ...
    if ($current_category_id > 0) {
      ...
      $categories_query = tep_db_query("select c.categories_id, if(length(cd1.categories_name), cd1.categories_name, cd.categories_name) as categories_name, c.categories_image, c.parent_id from " . TABLE_CATEGORIES_DESCRIPTION . " cd, " . TABLE_CATEGORIES . " c {$categories_join} left join " . TABLE_CATEGORIES_DESCRIPTION . " cd1 on cd1.categories_id = c.categories_id and cd1.language_id='" . (int)$languages_id ."' and cd1.affiliate_id = '" . (int)$HTTP_SESSION_VARS['affiliate_ref'] . "' where c.parent_id = '" . (int)$current_category_id . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' and c.categories_status = 1 and cd.affiliate_id = 0 order by sort_order, categories_name" . ($this->settings[0]['max_items'] ? " limit '" . (int)$this->settings[0]['max_items'] . "'" : ""));
      ...
      if (tep_db_num_rows($categories_query) > 0) {
        ...
        while ($categories = tep_db_fetch_array($categories_query)) {
          if (\common\helpers\Categories::count_products_in_category($categories['categories_id']) > 0 || Info::themeSetting('show_empty_categories')){
            ...
            $categories['link'] = tep_href_link('catalog', $cPath_new);
