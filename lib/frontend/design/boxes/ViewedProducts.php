...
class ViewedProducts extends Widget
{
  ...
  public function run()
  {
    ...
    if (isset($_SESSION['viewed_products'])) {
      ...
      $viewed_arr = explode(",", $_SESSION['viewed_products']);
      $viewed = '';
      foreach ($viewed_arr as $item) {
        if ($item != $get['products_id']) {
          if ($viewed) $viewed .= ",'" . (int)$item . "'";
          else $viewed = "'" . (int)$item . "'";
        }
      }
