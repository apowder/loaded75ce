...
<div class="cart-listing">
  <div class="headings">
    <div class="remove">{$smarty.const.TEXT_REMOVE_CART}</div>
    <div class="image">{$smarty.const.PRODUCTS}</div>
    <div class="name"></div>
    <div class="qty">{$smarty.const.PRICE}</div>
    <div class="price"></div>
  </div>

  {foreach $products as $product}
    <div class="item">
      <div class="remove">{if $product.remove_link}<a href="{$product.remove_link}" class="remove-btn"></a>{/if}</div>
      <div class="image">
        {if $product.status}
