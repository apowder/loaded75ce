...
<div class="checkout-login-page">
                ...
                {if in_array(ACCOUNT_DOB, ['required_register', 'visible_register'])}
                    <div class="col-left">
                        ...
                        <div class="" style="position: relative">
                            <input type="text" name="dob" id="dob" class="datepicker" value="{$customers_dob|escape:'html'}" {if ACCOUNT_DOB == 'required_register'}data-required="{$smarty.const.ENTRY_DATE_OF_BIRTH_ERROR}"{/if} />
                        </div>
                    </div>
                {/if}
                <div class="col-right">
    ...
    {if $show_socials}
        <div class="login-box">
            <div class="login-box-heading">{$smarty.const.TEXT_SOCIAL}</div>
            <div class="middle-form">
                {\yii\authclient\widgets\AuthChoice::widget(['baseAuthUrl'=>['account/auth'],'popupMode'=>false])}
            </div>
        </div>
    {/if}
</div>
...
<script type="text/javascript">
    ...
    {*input_email_address.on('keyup', function(){
    input_email_address.val($(this).val())
    })*}
    })
