...
<div class="site_width">
    ...
    <div class="tabsLogin">
        <div id="tab1" class="active">
            <form action="{tep_href_link(FILENAME_DEFAULT, 'action=process', 'SSL')}" method="post" name="login" onsubmit="return checkTerms();">
            <div class="login_title">{$smarty.const.TEXT_SIGN_IN_ACCOUNT}</div>
            <input type="hidden" name="account_login" value="login">
            <div class="login_form">
                <label for="email_address">{field_label const="TEXT_USERNAME" required_text=""}</label>
                <input type="text" name="email_address" id="email_address" data-required="{$smarty.const.EMAIL_REQUIRED}" placeholder="{$smarty.const.TEXT_DEMO_USER}"/>
            </div>
                <p class="entity_pass">{$smarty.const.TEXT_ENTITY_USERNAME}</p>
            <div class="login_form">
                <label for="password">{field_label const="ENTRY_PASSWORD" required_text=""}</label>
                <input type="password" name="password" id="password"/>
            </div>
            <div class="home_password_forgot"><a href="#tab3" class="logLink">{$smarty.const.TEXT_FORGOT_PASSWORD}</a></div>
            <div class="login_btns after">
                <div class="colL">
                    <input type="checkbox" id="terms" name="terms">
                    <label for="terms"><strong>{$SMARTY.CONST.ACCEPT}</strong> {$smarty.const.TEXT_TERMS_CONDITIONS}</label>
                </div>
                <div class="colR"><button class="btn" type="submit">{$smarty.const.HEADER_TITLE_LOGIN}</button></div>
            </div>
            </form>
        </div>
        <div id="tab2">
    ...
    <ul class="tabsRegister">
            ...
            <li><a href="#tab2">{$smarty.const.TEXT_DONT_HAVE_USERNAME}</a></li>
...
<script type="text/javascript" src="{Info::themeFile('/js/jquery-2.2.0.min.js')}"></script>
<script type="text/javascript">
  tl(function(){
	...
	function checkTerms() {
	if (document.login.terms.checked) {
		return true;
	}
