  ...
  {if $to_pdf}
    ...
    {foreach $order->products as $product}
      <tr>
        ...
        <td style="width: {($width*0.0809)|ceil}px">{$currencies->format($product.final_price, true, $order->info['currency'], $order->info['currency_value'])}</td>
        <td style="width: {($width*0.01)|ceil}px">{$currencies->format($product.final_price * $product.qty, true, $order->info['currency'], $order->info['currency_value'])}</td>
        <td style="width: {($width*0.11)|ceil}px; padding-right: 0"><b>{$currencies->format(\common\helpers\Tax::add_tax($product.final_price, $product.tax) * $product.qty, true, $order->info['currency'], $order->info['currency_value'])}</b></td>
      </tr>
    {/foreach}
  ...
  {else}
    ...
    {foreach $order->products as $product}
      <tr>
        ...
        <td>{$currencies->format($product.final_price, true, $order->info['currency'], $order->info['currency_value'])}</td>
        <td>{$currencies->format($product.final_price * $product.qty, true, $order->info['currency'], $order->info['currency_value'])}</td>
        <td><b>{$currencies->format(\common\helpers\Tax::add_tax($product.final_price, $product.tax) * $product.qty, true, $order->info['currency'], $order->info['currency_value'])}</b></td>
      </tr>
    {/foreach}
