...
<div class="functionality-bar">
  <form action="{$sorting_link}" method="get" class="sort-form">
    ...
    <div class="view">
      ...
      {if $grid_link}<a href="{$grid_link}" class="grid{if $gl == 'grid'} active{/if}" title="{$smarty.const.TEXT_GRID_VIEW}"></a>{/if}
      {if $list_link}<a href="{$list_link}" class="list{if $gl == 'list'} active{/if}" title="{$smarty.const.TEXT_LIST_VIEW}"></a>{/if}
      {if $b2b_link}<a href="{$b2b_link}" class="b2b{if $gl == 'b2b'} active{/if}" title="{$smarty.const.TEXT_B2B_VIEW}"></a>{/if}
    </div>
