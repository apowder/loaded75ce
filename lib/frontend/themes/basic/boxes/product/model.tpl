...
{if $data.ean && $settings.show_ean != 'no'}
  <div class="ean">
    <strong>{$smarty.const.TEXT_EAN}<span class="colon">:</span></strong> <span>{$data.ean}</span>
  </div>
{/if}
...
{if $data.upc && $settings.show_upc != 'no'}
  <div class="upc">
    <strong>{$smarty.const.TEXT_UPC}<span class="colon">:</span></strong> <span>{$data.upc}</span>
  </div>
{/if}