...
class XmlsitemapController extends Sceleton
{
  ...
  protected $config = array(
    ...
    'GSM_INFOPAGE_CHANGE_FREQ' => 'weekly',

    'GSM_PROD_MIN_RATIO' => '1.0',
    'GSM_CAT_MIN_RATIO' => '0.9',
    'GSM_BRAND_MIN_RATIO' => '0.8',
    'GSM_NEWS_MIN_RATIO' => '0.8',
    'GSM_INFOPAGE_MIN_RATIO' => '0.8',

    'GSM_PROD_PAGE_COUNT' => '100',
  );
...
//      ),
  ...
  public function actionIndex()
  {
    ...
    foreach ( $this->registered_map as $registered_feed ) {
      ...
      if ($registered_feed['feedname'] == 'products' || $registered_feed['feedname']=='images' ) {
        $feed_count = 1;
  ...
  protected function do_create_categories( $pageNumber=0 ){
    ...
    $categories_r = tep_db_query(
      ...
      "ORDER BY parent_id, sort_order, categories_id "
    );
    while ( $categories = tep_db_fetch_array($categories_r) ) {
      ...
      $location = tep_href_link(/*FILENAME_DEFAULT */'catalog/index', $strCategoryPath, 'NONSSL',false);
  ...
  protected function do_create_products( $pageNumber=0 ){
    global $languages_id, $customer_groups_id, $currency_id;
    ...
    $shop_products_sql = (
      "SELECT p.products_id, p.products_last_modified, p.products_date_added, p2c.categories_id, ".$ratio_column." as line_ratio ".
    ...
    do{
      $process_pages = true;
      $pass_page++;
      $shop_products_sql_page = $shop_products_sql . " LIMIT ".($page_fetch*($pass_page-1)).", {$page_fetch}";
  ...
  protected function do_create_manufacturers( $pageNumber=0 ){
    ...
    while( $used_manufacturers = tep_db_fetch_array($get_used_manufacturers_r) ) {
      $location = tep_href_link(/*FILENAME_DEFAULT*/'catalog/index', 'manufacturers_id='.$used_manufacturers['manufacturers_id'], 'NONSSL',false);
  ...
  protected function do_create_pages( $pageNumber=0 ){
    ...
    while ($row = tep_db_fetch_array($sql_query)){
      ...
      if (true || $row['page'] == ''){
        ...
        if ($row['page_type']=='SSL' || strpos($row['page'],'account')===0) continue;

        $location = tep_href_link($row['page'], '', $row['page_type'],'NONSSL',false);
      }
      ...
      while( $shop_products = tep_db_fetch_array($shop_products_r) ) {
        ...
        foreach ( Images::getImageList($shop_products['products_id']) as $_image){
          ...
          if ( !empty($_image['title']) ) {
            $images_set[$uri]['image:caption'] = $_image['title'];
          }
        }

        if ( count($images_set)>0 ) {
          foreach( $images_set as $idx=>$images_set_patch ) {
            $images_set[$idx]['image:loc'] = HTTP_SERVER.self::__xml_site_map_encode_image_path($images_set_patch['image:loc']);
          }
      ...
      foreach( $url['images_set'] as $image ) {
        $media .=
          ...
          ((isset($image['image:caption']) && !empty($image['image:caption']))?
            ...
            "\t\t\t<image:caption>{$image['image:caption']}</image:loc>\n":''
          ).
  ...
  static protected function __xml_site_map_encode_image_path($image_path){
  ...
  static protected function _get_max_ratio_param_in_categories( $ratio_param='viewed', $categories_ids=false ) {
    ...
    if ( isset( $HTTP_SESSION_VARS['affiliate_ref'] ) && (int)$HTTP_SESSION_VARS['affiliate_ref']>0 ) {
  ...
  static protected function _get_categories_map_tree(){
    ...
    if ( $categoriesMap===false ) {
      ...
      $categories_tree_sql =
        ...
        "left join " . TABLE_CATEGORIES_DESCRIPTION .  " cd1 on cd1.categories_id = cd.categories_id and cd1.affiliate_id = '" . (int)$HTTP_SESSION_VARS['affiliate_ref'] . "' and cd1.language_id = '" . (int)$languages_id . "' ".
